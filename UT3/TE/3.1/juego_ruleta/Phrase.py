...
class Phrase:
    ...
    def getPhrase(category: str = None) -> 'Phrase':
        ...
        if not category is None:
            frases_candidatas = PhraseModel.select().where(PhraseModel.category == category)
        else:
            frases_candidatas = PhraseModel.select()

        if len(frases_candidatas) == 0:
            raise ValueError("No se ha podido obtener ninguna frase")
