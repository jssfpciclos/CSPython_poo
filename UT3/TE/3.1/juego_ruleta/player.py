...
class Player(ABC):  # Clase padre Player
    ...
    # @abstractmethod  # Metodo abstracto hay que importarlo de abc abstractmethod
    @classmethod
    def FromSavedPlayer(cls, name: str, total_money: int, tipo: TipoJugadorEnum.HUMANO):

        match tipo:
            case TipoJugadorEnum.HUMANO:
                player = HumanPlayer(name)
                player.prizeMoney = total_money
                return player

            case TipoJugadorEnum.COMPUTADORA:
                player = ComputerPlayer(name)
                player.prizeMoney = total_money
                return player

            case TipoJugadorEnum.EQUIPO:
